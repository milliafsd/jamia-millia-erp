...
# ==========================================
# 1. ڈیٹا بیس سیٹ اپ (نئے سرے سے)
# ==========================================
DB_NAME = 'jamia_millia_app.db'
...
def clean_text(val):
    """ناموں کے ساتھ آنے والے فالتو بریکٹ اور کوما صاف کرنے کا فنکشن"""
    if not val: return ""
    cleaned = re.sub(r"[()\'\",]", "", str(val))
    return cleaned.strip()

def init_db():
    # تمام ٹیبلز مکمل کالمز کے ساتھ
...
<style>
    ...
    * { font-family: 'Noto Nastaliq Urdu', serif !important; direction: rtl; text-align: right; }
    .main-header { text-align: center; color: white; background-color: #1e5631; padding: 20px; border-radius: 10px; margin-bottom: 20px; }
...
def generate_html_print(dataframe, title):
    ...
    return f'<a href="data:text/html;base64,{b64}" download="{title}.html" style="text-decoration:none;"><button style="background-color:#008CBA; color:white; padding:10px; border:none; border-radius:5px; cursor:pointer; width:100%;">🖨️ پرنٹ / ڈاؤنلوڈ کریں</button></a>'

def execute_delete(table_name, record_id):
    c.execute(f"DELETE FROM {table_name} WHERE id=?", (record_id,))
    conn.commit()
    st.success("ریکارڈ کامیابی سے حذف کر دیا گیا!")
    st.rerun()
...
# ==========================================
# ماڈیول: انتظامی کنٹرول
# ==========================================
if m == "⚙️ انتظامی کنٹرول":
    st.header("⚙️ انتظامی کنٹرول (اساتذہ اور طلباء)")
    ...
    with tab1:
        ...
        with st.form("add_teacher"):
            ...
            if st.form_submit_button("محفوظ کریں"):
                c.execute("INSERT OR IGNORE INTO teachers (name, password) VALUES (?,?)", (t_name, t_pass))
                conn.commit(); st.success("استاد رجسٹر ہو گیا!")
                st.rerun()
        
        st.subheader("موجودہ اساتذہ")
        df_t = pd.read_sql_query("SELECT id, name FROM teachers WHERE name != 'admin'", conn)
...
# ==========================================
    ...
    if not students: st.warning("آپ کی کلاس میں کوئی طالب علم نہیں ہے۔")
    else:
        for s_raw, f_raw in students:
            # ناموں کو صاف کرنا (Clean Text)
            clean_s = clean_text(s_raw)
            clean_f = clean_text(f_raw)
            
            with st.expander(f"👤 طالب علم: {clean_s} ولد {clean_f}"):
                att = st.radio(f"حاضری", ["حاضر", "غیر حاضر", "رخصت"], key=f"att_{clean_s}", horizontal=True)
                
                if att == "حاضر":
                    st.markdown("### 📖 نیا سبق")
                    s_nagha = st.checkbox("سبق کا ناغہ", key=f"sn_{clean_s}")
                    if not s_nagha:
                        c1, c2, c3 = st.columns([2, 1, 1])
                        surah = c1.selectbox("سورت", surahs_urdu, key=f"surah_{clean_s}")
                        a_from = c2.text_input("آیت سے", key=f"af_{clean_s}")
                        a_to = c3.text_input("آیت تک", key=f"at_{clean_s}")
                        sabq_final = f"{surah}: {a_from}-{a_to}"
                    else: sabq_final = "ناغہ"

                    st.markdown("### 🔄 سبقی")
                    sq_nagha = st.checkbox("سبقی کا ناغہ", key=f"sqn_{clean_s}")
                    sq_list, sq_err, sq_atk = [], 0, 0
                    if not sq_nagha:
                        if f"sq_c_{clean_s}" not in st.session_state: st.session_state[f"sq_c_{clean_s}"] = 1
                        for i in range(st.session_state[f"sq_c_{clean_s}"]):
                            c1, c2, c3, c4 = st.columns([2, 2, 1, 1])
                            p = c1.selectbox(f"پارہ", paras, key=f"sqp_{clean_s}_{i}")
                            v = c2.selectbox(f"مقدار", ["مکمل", "آدھا", "پون", "پاؤ"], key=f"sqv_{clean_s}_{i}")
                            a = c3.number_input(f"اٹکن", 0, 20, key=f"sqa_{clean_s}_{i}")
                            e = c4.number_input(f"غلطی", 0, 20, key=f"sqe_{clean_s}_{i}")
                            sq_list.append(f"{p}:{v}"); sq_atk += a; sq_err += e
                        if st.button("➕ مزید سبقی", key=f"add_sq_{clean_s}"): st.session_state[f"sq_c_{clean_s}"] += 1; st.rerun()
                    else: sq_list = ["ناغہ"]

                    st.markdown("### 🏠 منزل")
                    m_nagha = st.checkbox("منزل کا ناغہ", key=f"mn_{clean_s}")
                    m_list, m_err, m_atk = [], 0, 0
                    if not m_nagha:
                        if f"m_c_{clean_s}" not in st.session_state: st.session_state[f"m_c_{clean_s}"] = 1
                        for i in range(st.session_state[f"m_c_{clean_s}"]):
                            c1, c2, c3, c4 = st.columns([2, 2, 1, 1])
                            p = c1.selectbox(f"پارہ", paras, key=f"mp_{clean_s}_{i}")
                            v = c2.selectbox(f"مقدار", ["مکمل", "آدھا", "پون", "پاؤ"], key=f"mv_{clean_s}_{i}")
                            a = c3.number_input(f"اٹکن", 0, 20, key=f"ma_{clean_s}_{i}")
                            e = c4.number_input(f"غلطی", 0, 20, key=f"me_{clean_s}_{i}")
                            m_list.append(f"{p}:{v}"); m_atk += a; m_err += e
                        if st.button("➕ مزید منزل", key=f"add_m_{clean_s}"): st.session_state[f"m_c_{clean_s}"] += 1; st.rerun()
                    else: m_list = ["ناغہ"]

                    if st.button("محفوظ کریں", key=f"save_{clean_s}"):
                        c.execute("INSERT INTO hifz_records (r_date, s_name, f_name, t_name, surah, sq_p, sq_a, sq_m, m_p, m_a, m_m, attendance) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                                  (str(sel_date), clean_s, clean_f, st.session_state.username, sabq_final, " | ".join(sq_list), sq_atk, sq_err, " | ".join(m_list), m_atk, m_err, att))
                        conn.commit(); st.success("محفوظ ہو گیا!")
                else:
                    if st.button("حاضری محفوظ کریں", key=f"save_absent_{clean_s}"):
                        c.execute("INSERT INTO hifz_records (r_date, s_name, f_name, t_name, attendance, surah, sq_p, m_p) VALUES (?,?,?,?,?,?,?,?)", 
                                  (str(sel_date), clean_s, clean_f, st.session_state.username, att, "-", "-", "-"))
                        conn.commit(); st.success(f"{att} درج ہو گئی!")

# ==========================================
# ماڈیول: یومیہ تعلیمی رپورٹ
# ==========================================
...
# ==========================================
    ...
    if not s_list: st.warning("کوئی طالب علم نہیں۔")
    else:
...
# ماڈیول: امتحانات
# ==========================================
elif m in ["🎓 امتحانات و نتائج", "🎓 امتحان کے لیے نامزدگی"]:
            ...
            for eid, sn, fn, pn in pending:
                with st.expander(f"📝 {sn} - پارہ {pn}"):
                    ...
                    if st.button("محفوظ کریں", key=f"sv_{eid}"):
                        g = "ممتاز" if tot>=90 else "مقبول"
                        c.execute("UPDATE exams SET total=?, grade=?, status='مکمل', end_date=? WHERE id=?", (tot, g, str(date.today()), eid))
                        conn.commit(); st.rerun()
        with tab2:
            df_exams = pd.read_sql_query("SELECT id as 'ID', s_name as 'نام', f_name as 'ولدیت', para_no as 'پارہ', total as 'نمبر', grade as 'گریڈ' FROM exams WHERE status='مکمل'", conn)
            ...
            st.divider()
            del_ex = st.number_input("حذف کرنے کے لیے ID درج کریں", min_value=0, step=1, key="del_ex")
...
# ==========================================
    ...
    st.divider()
    del_att = st.number_input("حذف کرنے کے لیے ID درج کریں", min_value=0, step=1, key="del_att")
...
# ==========================================
    ...
    for rid, tn, lt, d, rsn, sd in reqs:
        with st.expander(f"درخواست: {tn} ({lt} - {d} دن)"):
            ...
            c1, c2 = st.columns(2)
            if c1.button("✅ منظور", key=f"app_{rid}"):
                c.execute("UPDATE leave_requests SET status='منظور' WHERE id=?", (rid,)); conn.commit(); st.rerun()
            if c2.button("❌ مسترد", key=f"rej_{rid}"):
                c.execute("UPDATE leave_requests SET status='مسترد' WHERE id=?", (rid,)); conn.commit(); st.rerun()
    st.divider()
    df_lv = pd.read_sql_query("SELECT id as 'ID', t_name as 'استاد', l_type as 'نوعیت', days as 'دن', start_date as 'تاریخ', status as 'سٹیٹس' FROM leave_requests WHERE status!='پینڈنگ'", conn)
    st.dataframe(df_lv, use_container_width=True)
    if not df_lv.empty: st.markdown(generate_html_print(df_lv.drop(columns=['ID']), "رخصت کا ریکارڈ"), unsafe_allow_html=True)
    st.divider()
    del_lv = st.number_input("حذف کرنے کے لیے ID درج کریں", min_value=0, step=1, key="del_lv")
    if st.button("رخصت حذف کریں"): execute_delete("leave_requests", del_lv)
